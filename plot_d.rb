...
min_neg_log_p_value = ARGV[3].to_i
...
(d_matrix.size).times do |z1|
	...
	(d_matrix.size).times do |z2|
		...
		opacity = 0
		if p_matrix[z1][z2] < 10**(-min_neg_log_p_value)
			opacity = 1.0
		else
			neg_log_p = -Math.log(p_matrix[z1][z2],10)
			opacity = [(neg_log_p-1)/(min_neg_log_p_value-1).to_f,0].max
		end
		if opacity > 0
...
# end
y = legend_y + legend_label_shift3
texts << Text.new(x,y,font,legend_font_size,"normal","black","-#{0}","end","none")
y = legend_y + legend_label_shift3 + (legend_height-legend_label_shift3)
texts << Text.new(x,y,font,legend_font_size,"normal","black","-#{min_neg_log_p_value}","end","none")
