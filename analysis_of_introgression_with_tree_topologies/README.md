...
* Execute the script `make_alignments_from_maf.py` to extract up to a maximum of 10,000 alignments (`-n 10000`) with a length of 1,000 bp (`-l 1000 -k 1000`) in Nexus format (`-f nexus`) from the whole-genome alignment `cichlids_chr5.maf`, using only those with sequences for all six species of the whole-genome alignment (`-x 6`), a completeness of at least 95% (`-c 0.95`), and at least 20 polymorphic sites (`-m 10`):
	...
	The last line of the screen output should indicate that close to 6,000 alignments have been written to directory `short_alignments`.
